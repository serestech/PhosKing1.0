...
This time it splits IDs for each dataset partition (Train/Valid/Test) and the assigns data.
'''
import argparse
...
IDs = np.unique([ID for ID,_,_ in dataset.data])
train_IDs, rest_IDs = train_test_split(IDs, train_size=0.8, test_size=0.2, shuffle=True)
test_IDs, validation_IDs = train_test_split(rest_IDs, train_size=0.5, test_size=0.5, shuffle=True)

train_idx = []
validation_idx = []
test_idx = []
for idx, (ID, _, _) in enumerate(dataset.data):
    if ID in train_IDs:
...
print(f'Number of sequences:\n   Training {len(train_IDs)}  Validation: {len(validation_IDs)}  Test: {len(test_IDs)}')
print(f'Number of observations:\n   Training {len(train_idx)}  Validation: {len(validation_idx)}  Test: {len(test_idx)}')

assert set(train_idx).isdisjoint(test_idx) and set(test_idx).isdisjoint(validation_idx)  # Sanity check
...
validation_every_steps = int(int(len(train_idx)/batch_size)/validations_per_epoch) if args.mode == 'phospho' else 10
