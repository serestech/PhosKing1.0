...
class ESM_Embeddings(data.Dataset):
    ...
    def __init__(self, fasta: str, features: str, embeddings_dir: str,
                 idx_file, train, valid, test, frac_phos=0.5,
        ...
        self._load_metadata(self.features_file_name, frac_phos, idx_file, train, valid, test)
    ...
    def _load_metadata(self, features_file_name, frac_phos, idx_file, train, valid, test):
        '''
        ...
        ID_splits = {}
        key = None
        with open(idx_file, 'r') as infile:
            for line in infile:
                if line.startswith('%'):
                    ...
                    ID = line.strip()
                    if ID in self.IDs:
                        ID_splits[key].add(ID)

        self.train_IDs = set().union(*[ID_splits[s] for s in train])
        self.validation_IDs = set().union(*[ID_splits[s] for s in valid])
        self.test_IDs = set().union(*[ID_splits[s] for s in test])
        self.train_idx = []
        self.validation_idx = []
        self.test_idx = []
