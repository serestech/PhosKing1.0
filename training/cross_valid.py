...
From train_model3.py, Cross-Validation.
'''
import argparse
...
from dataset_cv import ESM_Embeddings
import time as t
...
parser.add_argument('-f', '--fasta', action='store', dest='fasta_path',
                    default=os.path.abspath(f'{HERE}/../data/train_data/sequences.fasta'),
                    help='Fasta file')
parser.add_argument('-ft', '--features', action='store', dest='features_path',
                    default=os.path.abspath(f'{HERE}/../data/train_data/features.tsv'),
...
parser.add_argument('-if', '--idx_file', action='store', dest='idx_file',
                    required=True,
                    help='File with the sequence partitions for cross_validation folds')
parser.add_argument('-i', '--idx_n', action='store', dest='idx_n',
                    required=True, type=int,
                    help='Iteration number in the cross_validation')
parser.add_argument('-p', '--partition', action='store', dest='partition',
                    nargs=3, type=int, default=[8, 1, 1],
                    help='Train, Validation and Test number of splits')
...
parser.add_argument('-vs', '--val_steps', action='store', type=int,
                    dest='val_steps', default=250,
                    help='Perform validation every this number of training steps')
parser.add_argument('-sf', '--save_final', action='store_true',
                    dest='save_final',
                    help='Saves the final model as _final.pth')


def perf_scores(preds, targets):
    '''
    Calculates performance scores from torch tensors
    '''
    ...
    total = preds.shape[0]
    pos = np.sum(preds_round)
    neg = total - pos
    true = np.sum(targets)
    false = total - true

    true_pos = np.sum(np.logical_and(preds_round, targets))
    false_pos = pos - true_pos
    true_neg = total - np.sum(np.logical_or(preds_round, targets))
    false_neg = neg - true_neg
    
    corrects = true_pos + true_neg
    mistakes = false_pos + false_neg

    accuracy = corrects / total

    try:
        sensitivity = true_pos / true
    except ZeroDivisionError as err:
        print(f'WARNING: {err} in sensitivity; {true_pos=}', file=sys.stderr)
        sensitivity = 0
        
    try:
        specificity = true_neg / false
    except ZeroDivisionError as err:
    ...
    try:
        ...
        print(f'WARNING: {err} in AUC', file=sys.stderr)
        auc = 0

    try:
        prec_, rec_, thr_ = precision_recall_curve(targets, preds)
        auprc = 0
        for i in range(1, len(rec_)):
            auprc += abs(rec_[i-1] - rec_[i]) * (prec_[i] + prec_[i-1]) / 2
    except BaseException as err:
        print(f'WARNING: {err} in AUPRC', file=sys.stderr)
        auprc = 0

    return accuracy, precision, sensitivity, specificity, auc, auprc
...
n = args.idx_n - 1
tr, va, te = args.partition
tot = tr + va + te
train_splits = [(i+n)%tot+1 for i in range(tr)]
valid_splits = [(i+n)%tot+1 for i in range(tr, tr+va)]
test_splits = [(i+n)%tot+1 for i in range(tr+va, tot)]
...
validation_every_steps = args.val_steps
model.train()
for epoch in range(num_epochs):
    ...
    for inputs, targets in iter(train_loader):
        ...
        if step % validation_every_steps == 0:
            train_acc, train_prec, train_sens, train_spec, train_auc, train_auprc = perf_scores(train_outputs, train_targets)
            ...
            val_acc, val_prec, val_sens, val_spec, val_auc, val_auprc = perf_scores(val_outputs, val_targets)
            ...
            print(f'    Train AUC:      {train_auc:8.5f}   Train AUPRC: {train_auprc:8.5f}')
            print(f'    Valid AUC:      {val_auc:8.5f}   Valid AUPRC: {val_auprc:8.5f}')

            # Store best model if early stopping
...
test_acc, test_prec, test_sens, test_spec, test_auc, test_auprc = perf_scores(test_outputs, test_targets)
...
if args.save_final:
    save_model(model.state_dict(), model_type='final',
            ...
            test_sens=test_sens, test_auc=test_auc, test_auprc=test_auprc,
...
print(f'  AUPRCs:         Train {train_auprc:8.5f}   Valid {val_auprc:8.5f}   Test {test_auprc:8.5f}')
...
if early_stop:
    ...
    test_acc_best, test_prec_best, test_sens_best, test_spec_best, test_auc_best, test_auprc_best = perf_scores(test_outputs, test_targets)
    ...
    save_model(state_dict_best, model_type='best',
               ...
               test_sens=test_sens_best, test_auc=test_auc_best, test_auprc=test_auprc_best,
    ...
    print(f'  AUPRCs:         Train {train_auprc_best:8.5f}   Valid {val_auprc_best:8.5f}   Test {test_auprc_best:8.5f}')